...
Ограниченная модель данных и расширяемость GAMMA привели к поиску более гибкой,
но не менее мощной модели обработки запросов. Дезайн оператора,
используемый в базе данных GAMMA, предоставляет каждому оператору контроль
в рамках его собственного процесса, предоставляя сетевому ПО и операционной
системе синхронизацию нескольких операторов в отношениях производитель-потребитель
с использованием механизмов управления потоком (**flow-control**). Такой дизайн,
хотя и очень хорошо работает в GAMMA, не пригоден для одно-процессной оценки
запросов, поскольку несколько мест управления, то есть несколько операторов,
не могут быть реализованы внутри одного процесса без специальных
псевдо-многопроцессорных механизмов, таких как потоки. Поэтому концепции
оператора и передачи данных GAMMA не подходят для эффективного механизма
обработки запросов, предназначенного как для последовательного, так и для
параллельного выполнения запросов.

EXODUS [[7][7]] - это расширяемая база данных, в которой некоторые компоненты
следуют подходу «набору инструментов», в который входили: генератор оптимизатора
[[13][13]], [[14][14]] и язык реализации базы данных E [[27][27]], [[28][28]]
и другие компоненты, построенные как мощные, но не расширяемые компоненты,
например менеджер хранилища [[5][5]].

[5]: https://dl.acm.org/doi/10.5555/318826.318839 "M. J. Carey, D. J. DeWitt,
     J. E. Richardson, E. J. Shekita 'Object and File Management in the EXODUS
     Extensible Database System' in Proc. Conf. Very Large Data Bases,
     Kyoto, Japan, Aug. 1986, p.91"

[6]: ""

[7]: https://dl.acm.org/doi/10.5555/93490.94135 "M. J. Carey, David J. DeWitt, 
     G. Graefe, D. M. Haight, J. E. Richardson, D. T. Schuh, E. J. Shekita, 
     S. L. Vandenberg 'The EXODUS extensible DBMS project: an overview', in
...
Первоначально EXODUS задумывался как независимый от модели данных, то есть
предполагалось, что он будет поддерживать широкий спектр моделей данных, но
позже была разработана новая, мощная, структурно-объектно-ориентированная модель
данных под названием Extra [[6][6]]. Концепция независимости модели данных,
впервые исследованная в EXODUS, была сохранена в проекте Volcano. Во время
разработки менеджера хранилищ EXODUS были рассмотрены многие проблемы хранения и
доступа, исследованные в WiSS и GAMMA. Извлеченные уроки и компромиссы,
изученные в этих дискуссиях, безусловно, помогли сформировать идеи, лежащие в
основе Volcano. Дизайн и наработки E базы данных сильно повлияли на роль
итераторов для обработки запросов.

На наш дизайн оказали влияние и ряд других традиционных (реляционных) и
расширяемых систем. Ingres [[32][32]] и System R [[9][9]], вероятно, повлияли на
большинство баз данных, в частности их расширяемыми наследниками являются
проекты Starburst [[23][23]] и Postgres [[35][35]]. Интересно отметить, что
независимо от нашей работы группа Starburst также определила управляемую спросом
парадигму итератора в качестве подходящей основы для расширяемой архитектуры
оценки однопроцессных запросов после ее успешного использования в реляционной
системе System R, но пока не удалось совместить расширяемость с параллелизмом.
GENESIS [[1][1]] с самого начала подчеркивал важность унифицированных интерфейсов
операторов для расширяемости и возможности повторного использования кода.
...
XPRS был первым проектом, целью которого было объединить расширяемость и
параллелизм [[34][34]]. Его основная предпосылка - внедрить Postgres поверх
дисковых массивов RAID и операционной системы Sprite. XPRS и GAMMA в основном
отличаются по четырем направлениям. Во-первых, GAMMA поддерживает чисто
реляционную модель данных, а XPRS поддерживает расширяемую реляционную модель
Postgres. Во-вторых, основной формой параллелизма GAMMA является
внутриоператорный параллелизм на основе секционированных наборов данных.
